    ...
    setInterval(() => {
      if (button.parentElement) {
        button.parentElement.style.opacity = (
          parseFloat(getComputedStyle(button.parentElement).opacity) - 0.1
        ).toString();
...
const Gradient = (mode, color1, color2, rotation = "") => {
  ...
  return `${mode == "linear" ? "linear-gradient(" : "radial-gradient("}${
...
const parseFunctionClass = (functionClass) => {
  return functionClass.substring(
    functionClass.indexOf("(") + 1,
    functionClass.indexOf(")")
  );
};
...
const FunctionClass = (functionClass, element) => {
  ...
  if (head && head in functionClasses) {
    functionClasses[head](
      element,
      parseFunctionClass(functionClass).split(",")
    );
  }
};
  ...
  getAllElements().forEach((e) => {
    ...
    attr.forEach((a) => {
      if (
        window.matchMedia &&
        window.matchMedia(
          Object.values(mediaClasses)[Object.keys(mediaClasses).indexOf(a)]
        ).matches
      ) {
